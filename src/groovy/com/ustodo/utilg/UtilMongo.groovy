...
class UtilMongo {
    ...
    {
        return new com.mongodb.Mongo(Cfg.mongoip, Cfg.mongoport);
    }
    // Auth
...
// GET COLLECTION
    ...
    public static DBCollection getColl (String collname, boolean drop)
    {
        ...
        getCollFromScratch (Cfg.mongoip, Cfg.mongoport, Cfg.dbname, collname);
    }
    ...
    {
        getColl(Cfg.getCollNameThisUserFavs(username));
    }


    public static DBCollection getCollFreshCappedNot (String collname)
    {
        DB db = (new Mongo(Cfg.mongoip, Cfg.mongoport)).getDB(Cfg.dbname);
        UtilMongo.dropColl("getCollFreshCappedNot", collname, true)
...
//coll.validate();
    ...
    {
        DBCollection coll = (((new Mongo(ip, port)).getDB(namedb)).getCollection(collname));
    }
...
// GET RECORDS/DBO	SUBMIT QUERY
    ...
    {
        UtilMongo.getColl(collname).find(dboquery);
    }
    ...
    {
        O.o("130421 in updateRecord_UsingMongoFindAndModify")
        coll.findAndModify(dboWhere, dboValues)
    }
    ...
    public static void updateRecordTimestamp(String id, String username, String strDateInFileFormat)
    {
       try
       {
            ...
            def newtext2 = dboToUpdate.get("text")
    ...
    public static void updateRecordArchiveToggle (String id, String username,  boolean archive)
    {
        ...
        try
        {
            ...
            BasicDBObject dboToUpdate = getFlrById(id, username);
            //O.o "updating obj:" + dboToUpdate.toString();
            def dboArchivedUpdate = new BasicDBObject();
            if (dboToUpdate.archived)
            {
    ...
    public static void remove(String collname, String fieldname, String strSrchExact)
    {
        ...
        //com.ustodo.utilg.UtilMongo.getColl("favsckckck").remove (UtilMongo.cvtJsonStrToDboTest("{'date':'2012-01-18 05:23:06'}"))
        //		def rtn = [];
        //		BasicDBObject dboquery= new BasicDBObject(); // filelineraw compare
        //		dboquery.put("filelineraw", fileLineRawLike_piece)
    ...
    public static void removeFavsRecById(def id, String username)
    {
        BasicDBObject dboFlr = UtilMongo.getFlrById(id, username)
        if (dboFlr != null)
        ...
        int ipre = UtilMongo.getCollCnt(UtilMongo.getColl(Cfg.getCollNameThisUserFavs(username)))
        ...
        int ipost = UtilMongo.getCollCnt(UtilMongo.getColl(Cfg.getCollNameThisUserFavs(username)))
        if (ipre != (ipost+1))
    ...
    public static String[] getRecsFileLineRawEqual_StrArr(DBCollection coll, String fileLineRawLike_piece)
    {
        ...
        dboquery.put("filelineraw", fileLineRawLike_piece)
        DBCursor dbcursor = coll.find(dboquery);
        dbcursor.each  {rtn << it.filelineraw}
        rtn;
    }
...
//	{
//		getRecsFileLineRawEqual(UtilMongo.getColl(collname,fileLineRawLike_piece));
//	}
    ...
    public static ArrayList<DBObject> collAsList (String dbname, String collname, boolean stringify)
    {
        ...
        try
        {
            ...
            coll.find().eachWithIndex { it, i ->
                itcnt++;
                if (false && i < 3)
                    O.o "in collAsList first few:" + it.toString();
                if (stringify)
    ...
    //
    //		DBCollection dbcl = UtilMongo.getOrCreateDBAndColl("setWhiteboardNameVal", TodoController.getNameDB(), "configplatform");
    //		if (false)
    //			UtilMongo.removeAll("utilmongo.setWhiteboardNameVal", dbcl);
    //
    ...
    //		DBCollection dbcl = UtilMongo.getOrCreateDBAndColl("setWhiteboardNameVal", TodoController.getNameDB(), "configplatform");
    //		if (false)
    //			UtilMongo.removeAll("utilmongo.getWhiteboardNameVal", dbcl);
    //
    //		BasicDBObject doc = new BasicDBObject();
    //
    //		//doc.put("name", "MongoDB");
    ...
    public static BasicDBObject[] queryEquals(DBCollection coll, BasicDBObject dboquery)
    {
        def rtn = [];
        DBCursor dbcursor = coll.find(dboquery);
        dbcursor.each  {rtn << it}
    ...
    public static int writeUsToDoStdString(DBCollection dbc, String s)
    {
        BasicDBObject doc = new BasicDBObject();
    ...
    public static int putCommandHistory ( String username, String filelinerawstr_wDate)
    {
        ...
        def coll = UtilMongo.getOrCreateDBAndColl("db  write history",  Cfg.dbname, username+'History');
        coll.setWriteConcern(WriteConcern.FSYNC_SAFE);
    ...
    {removeAll(UtilMongo.getColl(collname))}
    ...
    {
        dropColl(desc, UtilMongo.getColl(collname), confirm)
    }
    ...
    {
        Mongo m = UtilMongo.getMongo();
        Set<String> sdb = m.getDatabaseNames();
        sdb.eachWithIndex { dbname, i ->
            O.o("schemaGetDBNames " + i + ".  [" + dbname + "] of [" + sdb.size() + "]");
        }
                                            ...
                                            int modPrint_NegAll_ZeroNone_posCount, String regex, boolean promptYN)
    {
        try
        {
            ...
            Set<String> colls = UtilMongo.schemaGetCollectionNames(dbname);
    ...
    {
        Mongo m = new Mongo( Cfg.mongoip, Cfg.mongoport);
        def fn = "/Users/hkon/sw/ustodo/favsckckck8.txt";
    ...
    public static String ensureIndex(DBCollection coll, String field) {
    ...
    )
    {
        ...
        try
        {
            O.oNoFilter "================ HK HK orderdir:" + orderdir;
            ...
            if (fieldToSrchOrNull != null)
            {
                ...
                {
                    dboq = UtilMongoDboCreate.buildDboAndLike(fieldToSrchOrNull, sArrTxtUpperLcaseSplit);
                }
            ...
            if (maputdoptionsOrNull_tm != null)
                minDateStr = UtilDate.buildMinDateStrFromMaxAge (maputdoptionsOrNull_tm)
            if (minDateStr != null)
            {
            ...
            def t = new Date();
            cur = coll.find(dboq); // .sort(["date" : 1]);
            O.oNoFilter("db find done in [" + UtilPerf.timeSince(t, "dbFind")+ "]")

            if (limitIfGt0 > 0)
            {
                cur = cur.limit(limitIfGt0)
                O.oNoFilter  "================ yes limit:" + limitIfGt0;
            }
            else
                O.oNoFilter  "================ no limit:" ;
            ...
            if (orderfieldOrNull != null)
            {
                ...
                O.oNoFilter  "================ orderdir:" + orderdir;
                O.oNoFilter  "================ orderfieldOrNull:" + orderfieldOrNull;
                dboorder.put(orderfieldOrNull, -1); // 1 ASC 2 DESC
    ...
    public static List<String> getAllUtdDatAsFLRStrList_deletemre ()
    {
        ...
        DBCursor cur = UtilMongo.getCur("favsckckck", dboq)
    ...
    public static List<String> getFLRStrList_gotmsg (String collname)
    {
        ...
        String rx = ".*msg::*";
        dboq.put("filelineraw",  java.util.regex.Pattern.compile(rx));
        DBCursor cur = UtilMongo.getCur(collname, dboq)
        ArrayList arrl = new ArrayList();
        while (cur.hasNext())
        {
            arrl.add(cur.next().get("filelineraw"))
    ...
    public static BasicDBObject getFlrById(def id, String username) {
        BasicDBObject dboq = new BasicDBObject ();
        ...
        //		String rx = ".*utdData.*";
        //		dboq.put("filelineraw",  java.util.regex.Pattern.compile(rx));
        DBCursor cur = UtilMongo.getCur(Cfg.getCollNameThisUserFavs(username), dboq)
        ArrayList arrl = new ArrayList();
        while (cur.hasNext())
        {
            dboFlrRtn = cur.next()
        ...
        return (BasicDBObject) arrl[0]; // array not needed, but as a template for a list fetcher
    }
    ...
    //            //		dbl.put(3, "Test4")
    //            //
    //            //		UtilMongo.put("insert from test", "insert from test UtilMongoMain", dbl)
    //
    //            // (2) works!  purpose: test order in db call
    //
    //
    //            //						DBCursor cur = UtilMongo.select("favsckckck", "filelineraw",
    ...
    //            if (true)
    //            {
    //                UtilMongo.testSoupToNuts()
    //            }
    //            if (false)
    //
    //            {
    //                BasicDBObject dboFlr = UtilMongo.getFlrById("4f55a52203646313003865c4", "ckckck")
    //                if (dboFlr != null)
    ...
    //            if (false)
    //            {
    //                BasicDBObject dboFlr = UtilMongo.removeFavsRecById("4f55a52203646313003865c4", "ckckck")
    //
    //            }
    //
    //
...
//UtilMongo.updateRecordTimestamp("51af7d33a6182033a24e3961", "bkon", "2013-06-05 13:59:53")
//
//
//2013-06-05 13:59:53
...
//
//        eėw
////   BasicDBObject dboq1 = UtilMongo.getDboLike("newText", "tester", false)
...
//    int a = al.size();
//
//    int b = com.ustodo.utilg.UtilMongo.getCollAndDBCountByName("console", "livetest", "favsckckck");
//    assert (a == b);
//
//}
//
...
////DBCollection dbc = UtilMongo.utilMongoGetColl("mongo groovy console", DB_NAME, COLL_NAME)
//
//
//
////System.exit(0);
...
//    DBCollection collread = UtilMongo.utilMongoCookStdDBandColl("mongo groovy console", NAME, "110725_090100")
//
//    collread.findOne().each() {
//        o "eee:" + it
//    }
//
//    //            DBCollection collread = UtilMongo.utilMongoGetColl("test read phase 2", DB_NAME, COLL_NAME);
//    BasicDBObject query = new BasicDBObject();
//    //query.put ("count", 1)
//
//    //    query.put ({ filelineraw : { $regex : /start.*shut/i, $nin : ['acmeblahcorp'] } })
