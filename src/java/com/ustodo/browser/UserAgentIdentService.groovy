...
class UserAgentIdentService extends WebTierService {
    ...
    def getUserAgentInfo()
    {
        ...
        if (agentInfo != null && agentInfo.agentString == userAgent) {
            ...
            log.warn "SecUser agent string has changed in a single session!"
            log.warn "Previous SecUser Agent: ${agentInfo.agentString}"
            log.warn "New SecUser Agent: ${userAgent}"
            log.warn "Discarding existing agent info and creating new..."
        } else {
            log.debug "SecUser agent info does not exist in session scope, creating..."
        }
    ...
    {
        com.ustodo.utilg.O.o ("in useragent:" + s)
    }

    public boolean isChrome()
    {
        common(getUserAgentInfo().browserType);
