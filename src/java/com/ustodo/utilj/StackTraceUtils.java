...
// import com.ustodo.utilj.StackTraceUtils
// from http://www.docjar.com/html/api/org/codehaus/groovy/runtime/StackTraceUtils.java.html
...
public class StackTraceUtils {
   ...
   public static Throwable sanitize(Throwable t) {
	   ...
	   if (!Boolean.getBoolean("groovy.full.stacktrace")) {
		   ...
		   if (false ) // original way - filter
		   {
			   ...
			   com.ustodo.utilg.O.o("pruned from [" + trace.length+ "] to [" + newTrace.size()+ "]");
			   // We don't want to lose anything, so log it
			   //STACK_LOG.log(Level.WARNING, "Sanitizing stacktrace:", t);
		   }
		   else // new way first X
		   {
			   for (int i = 0; i < 10; i++) {
				   StackTraceElement stackTraceElement = trace[i];
				   newTrace.add( stackTraceElement);
   ...
   public static void printSanitizedStackTrace(Throwable t, PrintWriter p) {
	   ...
	   StackTraceElement[] trace = t.getStackTrace();
	   //O.o("cleaned trace len:" + trace.length);
	   for (int i = 0; i < trace.length; i++) {
		   StackTraceElement stackTraceElement = trace[i];
		   com.ustodo.utilg.O.o(  "at "+stackTraceElement.getClassName()
   ...
   public static boolean isApplicationClass(String className) {
	   ...
	   for (int i = 0; i < GROOVY_PACKAGES.length; i++) {
		   ...
		   if (className.startsWith(groovyPackage)) {
			   {
				   //O.o ("stacktrace clean test FLASE/TOSS [" + className + "] against [" + groovyPackage+ "]");
				   return false;
			   }
		   }
	   }
	 //  O.o ("stacktrace clean test TRUE/KEEP [" + className + "] against [" + groovyPackage+ "]");
	   return true;
   }
